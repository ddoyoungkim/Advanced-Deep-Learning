...
from dataloader import TrajDataset, BucketSampler, collate_fn
...
def get_dataloader(fname, n_samples=None,n_processors=None, transform=None):
    ...
    bucketing = BucketSampler(sampler, 15000)
...
def main():
    ...
    # create log_file
    log_f = open('log/{}_train.txt'.format(config.name), 'w')
    log_error = open('log/{}_train_error.txt'.format(config.name), 'w')
    ...
    for epoch in range(EPOCHS):
        ...
        if saved_modelname: # the model improved on at least one of four tasks.
            ...
            torch.save(models_dict, 
                       os.path.join('models', saved_modelname+'.pt'))
        ...
        if (epoch+1) % config.save_epoch == 0:
            ...
            torch.save(models_dict, 
                       os.path.join('models', config.name + '_e{}'.format(epoch+1) +'.pt'))
        log_f.flush()
        # every epoch
        scheduler.step()
    log_f.close()
...
######################################################################
dataloader_destination = get_dataloader(val_fname,n_samples=284997,n_processors=9,transform=Destination())
dataloader_aug = get_dataloader(val_fname,n_samples=284997,n_processors=9,transform=Augmented())
dataloader_mask = get_dataloader(val_fname,n_samples=284997,n_processors=9,transform=Masked())
dataloader_perm = get_dataloader(val_fname,n_samples=284997,n_processors=9,transform=Permuted())

    
if __name__ == '__main__':
    
#     main()
    if opts.dataloader_destination :
        print("dataloader_destination is activated")
        sample_cnt = 0

        for i, batch in enumerate(dataloader_destination):
            if batch:
                sample_cnt += batch.tm_len.size(0)
                if i %100 == 99:
                    print("Scanning {} \nSample cnt:{}".format(i, sample_cnt))
            else : 
                print("batch all none, ignore the batch")
                
            
    elif opts.dataloader_aug :
        print("dataloader_aug is activated")
        sample_cnt = 0

        for i, batch in enumerate(dataloader_aug):
            if batch:
                sample_cnt += batch[0].tm_len.size(0)
                if i %100 == 99:
                    print("Scanning {} \nSample cnt:{}".format(i, sample_cnt))
            else : 
                print("batch all none, ignore the batch")
    elif opts.dataloader_mask :
        print("dataloader_mask is activated")
        sample_cnt = 0

        for i, batch in enumerate(dataloader_mask):
            if batch:
                sample_cnt += batch.tm_len.size(0)
                if i %100 == 99:
                    print("Scanning {} \nSample cnt:{}".format(i, sample_cnt))
    ...
    elif opts.dataloader_perm :
        print("dataloader_perm is activated")
        sample_cnt = 0

        for i, batch in enumerate(dataloader_perm):
            if batch:
                sample_cnt += batch[0].tm_len.size(0)
                if i %100 == 99:
                    print("Scanning {} \nSample cnt:{}".format(i, sample_cnt))
