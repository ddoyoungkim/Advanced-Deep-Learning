...
#         self.trajecotry_encoder_layer = TrajecotryEncoderLayer(config, firstlayer=True)
        ...
        self.rnn = nn.LSTM(input_size=config.hidden_size,
                 ...
                 num_layers=2, batch_first=True).to(config.device)
...
#         print('src_key_padding_mask', src_key_padding_mask.size(), src_key_padding_mask)
        ...
        try:
            traj_emb, _ = self.rnn(traj_emb)
        except:
            import pickle, sys
            traceback.print_exc()
            pickle.dump(traj_emb, open("rnn_error.pkl","wb"))
            try :
                traj_emb = nn.LSTM(input_size=config.hidden_size,
                 hidden_size=config.hidden_size,
                 num_layers=2, batch_first=True).to(torch.device('cpu'))(traj_emb.cpu())
            except:
                print("second error")
                traceback.print_exc()
            sys.exit()
        ...
        return pre_out, pre_out_tm, traj_emb
...
def compute_destination_loss(batch, traj_encoder,
    ...
    batch_out, out_tm, _ = traj_encoder(batch)
...
def compute_mask_loss(batch, traj_encoder, mask_proj, 
    ...
    return loss_mask, batch_queries,h_t, w_uh_t, negs, _neg_term,  
